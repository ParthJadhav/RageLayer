...
export const RAGELAYER_IGNORE_ATTR = "data-ragelayer-ignore";
...
export function defaultCaptureFilter(node: Node): boolean {
  ...
  if (el.hasAttribute(RAGELAYER_IGNORE_ATTR)) return false;
...
export function pinFixedDescendants(root: HTMLElement): () => void {
  ...
  for (const el of Array.from(root.querySelectorAll<HTMLElement>("*"))) {
    ...
    if (el.closest(`[${RAGELAYER_IGNORE_ATTR}]`)) continue;
