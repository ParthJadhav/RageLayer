...
.rl-bar {
  ...
  animation: rl-rise 0.28s cubic-bezier(0.2, 0.9, 0.3, 1.2);
}
@keyframes rl-rise {
...
.rl-btn:hover:not([aria-disabled="true"]) { background: rgba(255, 255, 255, 0.1); }
.rl-btn:active:not([aria-disabled="true"]) { transform: scale(0.94); }
.rl-btn[aria-pressed="true"] { background: rgba(220, 90, 31, 0.9); color: #fff; }
/* Keyboard operation depends on this ring being visible in every state. */
.rl-btn:focus-visible { outline: 2px solid #fff; outline-offset: 2px; }
.rl-btn[aria-disabled="true"] { opacity: 0.38; cursor: default; }
.rl-btn img { display: block; }
.rl-divider {
  width: 1px;
  height: 26px;
  margin: 0 2px;
