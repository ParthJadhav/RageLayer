 ...
 * else into the decals buffer; running these twice would only burn time (and
 * `getImageData` on a document-sized buffer burns a lot of it).
 */
const CTX_READ_ONLY = new Set([
  "getImageData",
  "createImageData",
  "measureText",
  "isPointInPath",
 ...
 *
 * This exists because tools draw decals *imperatively* through a plain 2D
 * context — `drawCrack(engine.surfaceCtx, …)` — and in live mode anything that
 * only reaches the visible canvas is erased by the next `recompose()`. Mirroring
 * at the context level keeps every tool (including third-party ones) persistent
 * without changing how a single one of them draws.
 *
 * One deliberate translation: `source-atop` becomes `source-over` on the decals
 * side. Tools use atop to clip a mark to surviving page pixels, but the decals
 * buffer is transparent — atop against it would draw nothing. The clip the tool
 * wanted still happens, once, when `recompose` composites the whole buffer with
 * `source-atop`.
 */
function teeContexts(
  ...
  return new Proxy(visible, {
    ...
    set(target, prop, value) {
      ...
      let mirrored = value;
      if (prop === "globalCompositeOperation" && value === "source-atop") {
        mirrored = "source-over";
      }
      try {
        Reflect.set(decals, prop, mirrored);
      } catch {
        /* see above */
      }
      return true;
