...
Svelte has no first-party component, and does not need one: the `<desktop-destroyer>` custom
...
`src/routes/+page.svelte` also shows the `use:desktopDestroyer` action, which turns any button
