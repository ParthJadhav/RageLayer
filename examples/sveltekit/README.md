...
Svelte has no first-party component, and does not need one: the `<rage-layer>` custom
...
`src/routes/+page.svelte` also shows the `use:rageLayer` action, which turns any button
